...
class HomeView(View):
    ...
    def get(self, request):
        ...
        labels = []
        data = []
        stockqueryset = Stock.objects.filter(is_deleted=False).order_by('-quantity_in_stock')
        
        for item in stockqueryset:
            labels.append(item.name)
            data.append(item.quantity_in_stock)  # Changed to quantity_in_stock
        ...
        # Gather stock details for the context
        stock_details = []
        for stock in stockqueryset:
            stock_details.append({
                'name': stock.name,
                'image': stock.image.url if stock.image else None,
                'price': stock.price,
                'quantity_in_stock': stock.quantity_in_stock,
                'created_at': stock.created_at.strftime('%Y-%m-%d %H:%M:%S'),  # Format datetime
            })
        ...
        context = {
            ...
            'stock_details': stock_details,  # Include stock details in context
        }
